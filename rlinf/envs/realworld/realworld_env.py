...
class RealWorldEnv(gym.Env):
    ...
    def _create_env(self, env_idx: int):
        ...
        # Propagate keyboard_reward_wrapper so the inner env knows to skip
        # its own keyboard listener when the outer wrapper handles rewards.
        override_cfg["keyboard_reward_wrapper"] = self.cfg.get(
            "keyboard_reward_wrapper", None
        )
        # Propagate use_terminal_reward_wrapper for terminal-based evaluation
        if self.cfg.get("use_terminal_reward_wrapper") is not None:
            override_cfg["use_terminal_reward_wrapper"] = self.cfg["use_terminal_reward_wrapper"]
        # Propagate reward-related config so the inner env uses the right mode.
        for key in ("reward_mode", "step_penalty", "success_reward", "failure_penalty"):
            if self.cfg.get(key) is not None:
                override_cfg[key] = self.cfg[key]
        env = gym.make(
        ...
        is_dummy = env.unwrapped.config.is_dummy
        ...
        # 添加终端奖励包装器支持 (allow in dummy mode for human evaluation)
        # Note: Using RosRobotEnv's internal keyboard evaluation instead of wrapper
        # if self.cfg.get("use_terminal_reward_wrapper", False):
        #     from rlinf.envs.realworld.common.wrappers.terminal_reward_wrapper import TerminalRewardDoneWrapper
        #     env = TerminalRewardDoneWrapper(env)

        if self.cfg.get("use_pose_wrappers", True):
