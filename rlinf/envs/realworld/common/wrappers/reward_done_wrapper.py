...
class BaseKeyboardRewardDoneWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env, reward_mode: str = "on_intervene"):
        super().__init__(env)
        self.reward_modifier = 0
        self.listener = KeyboardListener()
        self.reward_mode = reward_mode
        assert self.reward_mode in ["always_replace", "on_intervene"]
    ...
    def step(
        ...
        if last_intervened:
            info["success"] = updated_reward > 0
            info["failure"] = updated_reward < 0
            info["is_success"] = updated_reward > 0
        terminated = terminated or updated_terminated
