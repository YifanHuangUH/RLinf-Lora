...
class EmbodiedSACFSDPPolicy(EmbodiedFSDPActor):
    ...
    def _is_dsrl_enabled(self) -> bool:
        model_cfg = self.cfg.actor.model
        return bool(
            model_cfg.get("openpi", {}).get("use_dsrl", False)
            or model_cfg.get("smolvla", {}).get("use_dsrl", False)
            or model_cfg.get("pi05_lerobot", {}).get("use_dsrl", False)
            or model_cfg.get("use_dsrl", False)
        )
    ...
    def setup_model_and_optimizer(self, initialize_target=False) -> None:
        ...
        # Cast to uniform dtype before FSDP wrap (mixed bf16/f32 causes
        # "Must flatten tensors with uniform dtype" when sync_module_states=True).
        if self.torch_dtype is not None:
            module = module.to(dtype=self.torch_dtype)
            if initialize_target:
                target_module = target_module.to(dtype=self.torch_dtype)
        ...
        self.use_dsrl = self._is_dsrl_enabled()
    ...
    def forward_critic(self, batch):
        ...
        use_dsrl = self._is_dsrl_enabled()
        if use_dsrl:
