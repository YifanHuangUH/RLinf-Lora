...
class FSDPModelManager:
    ...
    def model_provider_func(self) -> torch.nn.Module:
        ...
        if use_gptq:
            ...
            try:
                ...
                is_vision2seq = type(model_config) in AutoModelForVision2Seq._model_mapping.keys()
            except ImportError:
                is_vision2seq = False

            auto_model_class = AutoModelForVision2Seq if is_vision2seq else AutoModelForCausalLM
    ...
    def build_optimizers(
        ...
        for name, param in model.named_parameters():
            # CRITICAL FIX: Force unfreeze DSRL components even if they appear frozen
            # This handles the case where FSDP wrapping resets requires_grad flags
            should_be_trainable = False
            for dsrl_filter in ["critic_image_encoder", "critic_state_encoder", "q_head", 
                               "dsrl_action_noise_net", "actor_image_encoder", "actor_state_encoder"]:
                if dsrl_filter in name:
                    should_be_trainable = True
                    break
            
            if should_be_trainable and not param.requires_grad:
                self.logger.warning(f"[FSDP Optimizer] Force unfreezing parameter: {name[:100]}")
                param.requires_grad = True
        ...
        # Debug: print matched parameters for each filter group
        self._logger.info(f"[FSDP] Parameter filter statistics:")
        for key, params in filtered_params_dict.items():
            self._logger.info(f"  {key}: {len(params)} parameters matched (filters: {param_filters[key]})")
        self._logger.info(f"  main: {len(main_optim_params)} parameters")
        
        # Debug: if critic has no matches, print all available parameter names containing 'critic' or 'q_head'
        if len(filtered_params_dict.get("critic", [])) == 0:
            self._logger.warning("[FSDP] Critic filter matched 0 parameters! Listing all critic-related params:")
            for name, param in model.named_parameters():
                if "critic" in name or "q_head" in name:
                    if param.requires_grad:
                        self._logger.info(f"  ✓ {name} (requires_grad=True)")
                    else:
                        self._logger.info(f"  ✗ {name} (requires_grad=False)")
        ...
        for key, params in filtered_params_dict.items():
            assert len(params) > 0, (
                f"optimizer {key=} is not match any params, with param_filters[{key}]={param_filters[key]}. "
                f"This usually means the model components are frozen or named differently. "
                f"Check the detailed parameter list above."
            )
            optimizers.append(
                torch.optim.Adam(
                    [
